...
TDEXLogSmgrInitWrite(bool encrypt_xlog, int key_len)
{
	...
	/*
	 * Generate the key once, in the postmaster. EXEC_BACKEND children re-run
	 * this hook but must adopt the postmaster's key from the key file, not
	 * generate a fresh one per backend.
	 */
#ifndef FRONTEND
