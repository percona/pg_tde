...
static RelKeyData* tde_create_rel_key(const RelFileLocator *rlocator, InternalKey *key, TDEMasterKey *master_key, bool is_key_decrypted);
...
static void pg_tde_set_db_file_paths(void);
...
static void pg_tde_write_key_map_entry(const RelFileLocator *rlocator, RelKeyData *enc_rel_key_data, TDEMasterKey *master_key);
 ...
 */
RelKeyData *
tde_create_rel_key(const RelFileLocator *rlocator, InternalKey *key, TDEMasterKey *master_key, bool is_key_decrypted)
{
	RelKeyData 	*rel_key_data;
	...
	strncpy(rel_key_data->master_key_name, master_key->keyInfo.keyName, MASTER_KEY_NAME_LEN);
 ...
 */
int32
pg_tde_write_map_entry(const RelFileLocator *rlocator, char *db_map_path, TDEMasterKey *master_key)
{
	File map_file = -1;
	int32 key_index = 0;
	...
	/* Open and vaidate file for basic correctness. */
	map_file = pg_tde_open_file(db_map_path, &master_key->keyInfo, false, O_RDWR | O_CREAT, &is_new_file, &curr_pos);
	prev_pos = curr_pos;
...
pg_tde_read_one_keydata(File keydata_file, int32 key_index, TDEMasterKey *master_key)
{
	...
	strncpy(enc_rel_key_data->master_key_name, master_key->keyInfo.keyName, MASTER_KEY_NAME_LEN);
...
pg_tde_write_key_map_entry(const RelFileLocator *rlocator, RelKeyData *enc_rel_key_data, TDEMasterKey *master_key)
{
	...
	key_index = pg_tde_write_map_entry(rlocator, db_map_path, master_key);
