...
pg_tde_crypt(uint64 start_offset, const char* data, uint32 data_len, char* out, RelKeysData* keys, const char* context)
{
	...
	for (uint64 batch_start_block = aes_start_block; batch_start_block < aes_end_block; batch_start_block += NUM_AES_BLOCKS_IN_BATCH)
	{
		...
		current_batch_bytes = ((batch_end_block - batch_start_block) * AES_BLOCK_SIZE)
										- (batch_no > 0 ? 0 : aes_block_no); /* first batch skips `aes_block_no`-th bytes of encKey */
		...
		{
			/* 
			 * As the size of encKey always is a multiple of 16 we 
			 * start from `aes_block_no`-th index of the encKey[]
			 * so N-th will be crypted with the same encKey byte despite
			 ...
			 * so the 10th data byte is encoded with the 4th byte of the 2nd encKey etc.
			 ...
			 * again, the 10th data byte is encoded with the 4th byte of the 2nd encKey etc.
			 */
				...
				uint32 data_len, RelKeysData* keys, const char* context)
{
	...
	for (uint32 i = 0; i < data_len; i += DATA_BYTES_PER_AES_BATCH)
	{
		batch_len = i < data_len && DATA_BYTES_PER_AES_BATCH < data_len ? 
						DATA_BYTES_PER_AES_BATCH : data_len;

		pg_tde_crypt(read_start_offset + i, read_data + i, batch_len, decrypted, keys, context);
		pg_tde_crypt(write_start_offset + i, decrypted, batch_len, write_data + i, keys, context);
	}
}
