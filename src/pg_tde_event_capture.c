...
PG_FUNCTION_INFO_V1(ddl_command_start_capture);
PG_FUNCTION_INFO_V1(ddl_command_end_capture);
...
/*
 * ddl_command_start_capture is an event trigger function triggered
 ...
 */
Datum
ddl_command_start_capture(PG_FUNCTION_ARGS)
{
 ...
 */
Datum
ddl_command_end_capture(PG_FUNCTION_ARGS)
{
	/* Ensure this function is being called as an event trigger */
	if (!CALLED_AS_EVENT_TRIGGER(fcinfo)) /* internal error */
		ereport(ERROR,
