...
{
    TDEMasterKeyId  principal_key_id;
...
extern void pg_tde_write_key_map_entry(const RelFileLocator *rlocator, RelKeyData *enc_rel_key_data, TDEMasterKeyInfo *principal_key_info);
...
extern TDEMasterKeyInfo *pg_tde_get_principal_key(Oid dbOid, Oid spcOid);
extern bool pg_tde_save_principal_key(TDEMasterKeyInfo *principal_key_info);
extern bool pg_tde_perform_rotate_key(TDEMasterKey *principal_key, TDEMasterKey *new_principal_key);
extern bool pg_tde_write_map_keydata_files(off_t map_size, char *m_file_data, off_t keydata_size, char *k_file_data);
extern RelKeyData* tde_create_rel_key(Oid rel_id, InternalKey *key, TDEMasterKeyInfo *principal_key_info);
extern RelKeyData *tde_encrypt_rel_key(TDEMasterKey *principal_key, RelKeyData *rel_key_data, const RelFileLocator *rlocator);
extern RelKeyData *tde_decrypt_rel_key(TDEMasterKey *principal_key, RelKeyData *enc_rel_key_data, const RelFileLocator *rlocator);
