...
program_help_ok('pg_tde_rewind');
program_version_ok('pg_tde_rewind');
program_options_handling_ok('pg_tde_rewind');
...
command_fails(
	...
	'too many arguments');
command_fails([ 'pg_tde_rewind', '--target-pgdata', $primary_pgdata ],
	'no source specified');
command_fails(
	[
		'pg_tde_rewind', '--debug',
